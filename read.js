...
function _asyncToGenerator(fn){return function(){var gen=fn.apply(this,arguments);return new Promise(function(resolve,reject){function step(key,arg){try{var info=gen[key](arg),value=info.value}catch(error){return void reject(error)}return info.done?void resolve(value):Promise.resolve(value).then(function(value){step('next',value)},function(err){step('throw',err)})}return step('next')})}}const bookState=new __WEBPACK_IMPORTED_MODULE_8__lib_state_manager__["a" /* default */](location.href),globalState=new __WEBPACK_IMPORTED_MODULE_8__lib_state_manager__["a" /* default */](__WEBPACK_IMPORTED_MODULE_9__lib_constants__["a" /* globalStateKey */]),nextPage=()=>{const a=bookState.read('currentPage'),b=bookState.read('totalPages');a<b&&bookState.write('currentPage',a+1)},previousPage=()=>{const a=bookState.read('currentPage');1<a&&bookState.write('currentPage',a-1)},clickHandler=a=>{a.clientX>=window.innerWidth/2?nextPage():previousPage()},fontHandler=a=>{const b=a.target.selectedOptions[0].textContent;globalState.write('font-family',b)},onCurrentPageChanged=a=>{const b=document.querySelector('#text'),c=(a-1)*(b.clientWidth+__WEBPACK_IMPORTED_MODULE_9__lib_constants__["b" /* columnGap */]);document.querySelector('#text').style.transform=`translateX(-${c}px)`,document.querySelector('#currentPage').textContent=a},onTitleChanged=a=>{document.querySelector('#title').textContent=a,document.title=a},onStyleChanged=(a,b)=>{const c=document.querySelector('#text');c.style[b]=a,bookState.write('totalPages',Math.ceil(c.scrollWidth/(c.clientWidth+__WEBPACK_IMPORTED_MODULE_9__lib_constants__["b" /* columnGap */])))},onTotalPagesChanged=(a,b,c)=>{const d=bookState.read('currentPage');bookState.write('currentPage',Math.floor(d/(c||a)*a)),document.querySelector('#totalPages').textContent=a},calculateTotalPages=()=>{const a=document.querySelector('#text');bookState.write('totalPages',Math.ceil(a.scrollWidth/(a.clientWidth+__WEBPACK_IMPORTED_MODULE_9__lib_constants__["b" /* columnGap */])))};_asyncToGenerator(function*(){const a=__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_4__lib_deserialize_url_params__["a" /* default */])(location.search),b=a.get('url'),c=yield __webpack_require__.i(__WEBPACK_IMPORTED_MODULE_5__lib_idb_or_worker__["a" /* default */])(b,__WEBPACK_IMPORTED_MODULE_6__lib_workers_markdown_to_html_worker___default.a),d=document.querySelector('#text');d.innerHTML=c,document.querySelector('#container').addEventListener('click',clickHandler),document.querySelector('#font').addEventListener('change',fontHandler),bookState.listen('currentPage',onCurrentPageChanged),bookState.listen('title',onTitleChanged),bookState.listen('totalPages',onTotalPagesChanged),['font-family'].forEach(function(e){return globalState.listen(e,onStyleChanged)}),yield bookState.bootstrap({title:a.get('title'),currentPage:1}),calculateTotalPages()})(),__webpack_require__.i(__WEBPACK_IMPORTED_MODULE_7__lib_register_service_worker__["a" /* default */])();
